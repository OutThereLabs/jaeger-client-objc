...
- (void)setMaxReceiveIPv4BufferSize:(uint16_t)max
{
	dispatch_block_t block = ^{
		...
		NSLog(@"%@ %u", THIS_METHOD, (unsigned)max);
...
- (void)setupSendAndReceiveSourcesForSocket4
{
	...
	dispatch_source_set_event_handler(send4Source, ^{ @autoreleasepool {
		...
		NSLog(@"send4EventBlock");
		NSLog(@"dispatch_source_get_data(send4Source) = %lu", dispatch_source_get_data(send4Source));
		...
		{
			NSLog(@"currentSend - waiting for address resolve");
			[self suspendSend4Source];
		}
		else if (currentSend->filterInProgress)
		{
			NSLog(@"currentSend - waiting on sendFilter");
			[self suspendSend4Source];
		}
		else
		{
	...
	dispatch_source_set_event_handler(receive4Source, ^{ @autoreleasepool {
		...
		NSLog(@"receive4EventBlock");
		
		socket4FDBytesAvailable = dispatch_source_get_data(receive4Source);
		NSLog(@"socket4FDBytesAvailable: %lu", socket4FDBytesAvailable);
	...
	dispatch_source_set_cancel_handler(send4Source, ^{
		...
		NSLog(@"send4CancelBlock");
		
		#if !OS_OBJECT_USE_OBJC
		NSLog(@"dispatch_release(send4Source)");
	...
	dispatch_source_set_cancel_handler(receive4Source, ^{
		...
		NSLog(@"receive4CancelBlock");
		
		#if !OS_OBJECT_USE_OBJC
		NSLog(@"dispatch_release(receive4Source)");
...
- (void)setupSendAndReceiveSourcesForSocket6
{
	...
	dispatch_source_set_event_handler(send6Source, ^{ @autoreleasepool {
		...
		NSLog(@"send6EventBlock");
		NSLog(@"dispatch_source_get_data(send6Source) = %lu", dispatch_source_get_data(send6Source));
	...
	dispatch_source_set_event_handler(receive6Source, ^{ @autoreleasepool {
		...
		NSLog(@"receive6EventBlock");
		
		socket6FDBytesAvailable = dispatch_source_get_data(receive6Source);
		NSLog(@"socket6FDBytesAvailable: %lu", socket6FDBytesAvailable);
	...
	dispatch_source_set_cancel_handler(send6Source, ^{
		...
		NSLog(@"send6CancelBlock");
		
		#if !OS_OBJECT_USE_OBJC
		NSLog(@"dispatch_release(send6Source)");
		dispatch_release(theSendSource);
		#endif
	...
	dispatch_source_set_cancel_handler(receive6Source, ^{
		...
		NSLog(@"receive6CancelBlock");
		
		#if !OS_OBJECT_USE_OBJC
		NSLog(@"dispatch_release(receive6Source)");
		dispatch_release(theReceiveSource);
		#endif
...
- (void)suspendSend4Source
{
	...
	{
		NSLog(@"dispatch_suspend(send4Source)");
...
- (void)suspendSend6Source
{
	...
	{
		NSLog(@"dispatch_suspend(send6Source)");
...
- (void)resumeSend4Source
{
	if (send4Source && (flags & kSend4SourceSuspended))
	{
		NSLog(@"dispatch_resume(send4Source)");
...
- (void)resumeSend6Source
{
	if (send6Source && (flags & kSend6SourceSuspended))
	{
		NSLog(@"dispatch_resume(send6Source)");
...
- (void)suspendReceive4Source
{
	...
	{
		NSLog(@"dispatch_suspend(receive4Source)");
...
- (void)suspendReceive6Source
{
	...
	{
		NSLog(@"dispatch_suspend(receive6Source)");
...
- (void)resumeReceive4Source
{
	if (receive4Source && (flags & kReceive4SourceSuspended))
	{
		NSLog(@"dispatch_resume(receive4Source)");
...
- (void)resumeReceive6Source
{
	if (receive6Source && (flags & kReceive6SourceSuspended))
	{
		NSLog(@"dispatch_resume(receive6Source)");
...
- (void)closeSocket4
{
	if (socket4FD != SOCKET_NULL)
	{
		NSLog(@"dispatch_source_cancel(send4Source)");
		dispatch_source_cancel(send4Source);
		
		NSLog(@"dispatch_source_cancel(receive4Source)");
...
- (void)closeSocket6
{
	if (socket6FD != SOCKET_NULL)
	{
		NSLog(@"dispatch_source_cancel(send6Source)");
		dispatch_source_cancel(send6Source);
		
		NSLog(@"dispatch_source_cancel(receive6Source)");
...
- (BOOL)bindToPort:(uint16_t)port interface:(NSString *)interface error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		NSLog(@"Binding socket to port(%hu) interface(%@)", port, interface);
	...
	if (err)
		NSLog(@"Error binding to port/interface: %@", err);
...
- (BOOL)bindToAddress:(NSData *)localAddr error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (useIPv4)
		{
			NSLog(@"Binding socket to address(%@:%hu)",
	...
	if (err)
		NSLog(@"Error binding to address: %@", err);
...
- (BOOL)connectToHost:(NSString *)host onPort:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		NSLog(@"Dispatching DNS resolve for connect...");
	...
	if (err)
		NSLog(@"Error connecting to host/port: %@", err);
...
- (BOOL)connectToAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
	...
	if (err)
		NSLog(@"Error connecting to address: %@", err);
             ...
             tag:(long)tag
{
	...
	[self asyncResolveHost:host port:port withCompletionBlock:^(NSArray *addresses, NSError *error) {
		...
		{
			NSLog(@"currentSend - address resolved");
			[self doPreSend];
		}
	}];
			...
			dispatch_async(sendFilterQueue, ^{ @autoreleasepool {
				...
				dispatch_async(socketQueue, ^{ @autoreleasepool {
					...
					if (sendPacket == currentSend)
					{
						...
						{
							NSLog(@"currentSend - silently dropped by sendFilter");
...
- (void)doSend
{
	...
	if (flags & kDidConnect)
	{
		...
		if (currentSend->addressFamily == AF_INET)
		{
			...
			NSLog(@"send(socket4FD) = %d", result);
		}
		else
		{
			result = send(socket6FD, buffer, length, 0);
			NSLog(@"send(socket6FD) = %d", result);
		}
	}
	else
	{
		...
		if (currentSend->addressFamily == AF_INET)
		{
			...
			NSLog(@"sendto(socket4FD) = %d", result);
		}
		else
		{
			result = sendto(socket6FD, buffer, length, 0, dst, dstSize);
			NSLog(@"sendto(socket6FD) = %d", result);
		}
	}
	...
	if (waitingForSocket)
	{
		...
		NSLog(@"currentSend - waiting for socket");
...
- (BOOL)receiveOnce:(NSError **)errPtr
{
	...
	if (err)
		NSLog(@"Error in beginReceiving: %@", err);
...
- (void)doReceive
{
	...
	{
		NSLog(@"Receiving is temporarily paused (pending filter operations)...");
	...
	if (doReceive4)
	{
		...
		NSLog(@"recvfrom(socket4FD) = %i", (int)result);
		...
		{
			NSLog(@"recvfrom(socket4FD) = %@", [self errnoError]);
			socket4FDBytesAvailable = 0;
			free(buf);
		}
	}
	else
	{
		...
		NSLog(@"recvfrom(socket6FD) -> %i", (int)result);
		...
		{
			NSLog(@"recvfrom(socket6FD) = %@", [self errnoError]);
			socket6FDBytesAvailable = 0;
			free(buf);
		}
	}
				...
				if (receiveFilterAsync)
				{
					...
					dispatch_async(receiveFilterQueue, ^{ @autoreleasepool {
						...
						dispatch_async(socketQueue, ^{ @autoreleasepool {
							...
							{
								NSLog(@"received packet silently dropped by receiveFilter");
							}
...
- (BOOL)createReadAndWriteStreams:(NSError **)errPtr
{
	...
	NSLog(@"Creating read and write stream(s)...");
...
- (CFReadStreamRef)readStream
{
	...
	{
		NSLog(@"Error creating CFStream(s): %@", err);
		return NULL;
	}
...
//		if (!r1 || !r2)
//		{
//			NSLog(@"Error setting voip type (IPv4)");
//			return NO;
//		}
//	}
//	
...
//		if (!r1 || !r2)
//		{
//			NSLog(@"Error setting voip type (IPv6)");
//			return NO;
//		}
//	}
//	
